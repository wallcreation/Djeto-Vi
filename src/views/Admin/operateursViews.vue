...
function openEditModal(operateur) {
  form.value = { ...operateur }
  numeroSuffixe.value = operateur.telephone.replace('+22901', '')
  showModal.value = true
}
function saveEdit() {
  if (!validateForm()) return

  const index = operateurs.value.findIndex((c) => c.id === editingId.value)
  if (index !== -1) {
    operateurs.value[index] = { ...form.value, id: editingId.value }
  }
  closeEditModal()
}
...
function viewDetails(operateur) {
  selectedoperateur.value = operateur
  showDetailsModal.value = true
}

function deleteoperateur(id) {
  if (confirm('Supprimer ce operateur ?')) {
    operateurs.value = operateurs.value.filter((c) => c.id !== id)
...
<template>
  <div class="p-6 space-y-4">
    <div class="flex justify-between items-center">
      <h1 class="text-2xl font-bold">Gestion des operateurs</h1>
      <button
    ...
    <div v-if="operateurs.length === 0" class="bg-yellow-100 text-yellow-800 p-4 rounded">
      Aucun operateur n’a été créé pour le moment.
    </div>
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
      <div
        ...
        :key="operateur.id"
        class="bg-white dark:bg-gray-800 p-4 rounded-xl shadow space-y-2 transform transition duration-300 hover:scale-105 hover:shadow-xl"
      >
        <h2 class="font-semibold text-lg dark:text-white">{{ operateur.nom }}</h2>
        <h2 class="font-semibold text-lg dark:text-white">{{ operateur.prenom }}</h2>
        <p class="text-sm text-gray-600 dark:text-gray-300">
          Téléphone : {{ operateur.telephone }}
        </p>
        <p class="text-sm text-gray-600 dark:text-gray-300">Équipe : {{ operateur.equipe }}</p>
        <button
          @click="viewDetails(operateur)"
          class="text-white flex items-end justify-end bg-[#978b8b9f] px-4 cursor-pointer mt-[1em] rounded-2xl"
        >
          Voir plus
        </button>
      </div>
    ...
    >
      <div class="bg-white dark:bg-gray-900 p-6 rounded-xl shadow max-w-md w-full space-y-4">
        <h2 class="text-xl font-semibold dark:text-white">
          {{ showEditModal ? 'Modifier le operateur' : 'Nouveau operateur' }}
        </h2>
        <form @submit.prevent="showEditModal ? saveEdit() : createoperateur()" class="space-y-3">
    ...
    <!-- Modal Détails -->
    <div
      v-if="showDetailsModal && selectedoperateur"
      class="fixed inset-0 bg-black/40 flex items-center justify-center z-50"
    >
      <div class="bg-white dark:bg-gray-900 p-6 rounded-xl shadow max-w-md w-full space-y-4">
        <h2 class="text-xl font-semibold dark:text-white">Détails du operateur</h2>
        <p class="dark:text-white"><strong>Nom :</strong> {{ selectedoperateur.nom }}</p>
        <p class="dark:text-white"><strong>Prénom :</strong> {{ selectedoperateur.prenom }}</p>
        <p class="dark:text-white">
          <strong>Téléphone :</strong> {{ selectedoperateur.telephone }}
        </p>
        <p class="dark:text-white">
          <strong>Jour de travail :</strong> {{ selectedoperateur.jour }}
        </p>
        <p class="dark:text-white"><strong>Équipe :</strong> {{ selectedoperateur.equipe }}</p>
        <p class="dark:text-white"><strong>Mot de passe :</strong> *********</p>

        <div class="flex justify-end space-x-2 pt-2">
          <button
            @click="openEditModal(selectedoperateur)"
            class="px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600"
          >
            Modifier
          </button>
          <button
            @click="deleteoperateur(selectedoperateur.id)"
